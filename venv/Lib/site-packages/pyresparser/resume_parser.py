...
class ResumeParser(object):
    ...
    def __get_basic_details(self):
        ...
        # extract designation
        try:
            self.__details['designation'] = cust_ent.get('Designation', None)
        except KeyError:
            pass

        # extract company names
        try:
            self.__details['company_names'] = cust_ent.get('Companies worked at', None)
        except KeyError:
            pass

        try:
            self.__details['experience'] = entities.get('experience', None)
            exp = utils.get_total_experience(entities.get('experience', []))
            self.__details['total_experience'] = round(exp / 12, 2) if exp else 0
        except KeyError:
